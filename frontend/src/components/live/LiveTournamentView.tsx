...
export function LiveTournamentView({ tId }: Props) {
    ...
    if (state.state === TournamentState.COMPLETED) {
        return (
            <div className="bg-white border-2 border-dashed border-[#16a34a]/30 text-center py-16 px-8">
                ...
                <a href={`/tournaments/${tId}`} className="bg-[#16a34a] text-white px-8 py-3 font-pixel text-xs no-underline hover:bg-primary-deep transition-colors inline-block border-2 border-primary-deep shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)]">
                    View Results →
                </a>
            </div>
        );
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-4 relative">
                <div className="sticky top-24 border-2 border-primary/40 shadow-[4px_4px_0px_0px_rgba(19,236,91,0.1)]" style={{ background: "var(--color-dark-green)" }}>
                    {/* Feed Header */}
                    <div className="px-4 py-3 border-b border-primary/20 flex items-center justify-between" style={{ background: "var(--color-bg-darkest)" }}>
                        <div className="flex items-center gap-2">
                            <span className="w-2 h-2 bg-primary rounded-full animate-pulse" />
                            <h3 className="font-pixel text-[10px] text-primary tracking-widest">▶ LIVE FEED</h3>
                        </div>
                        <span className="font-pixel text-[8px] text-primary/40">SEASON {tId}</span>
                    </div>
                    ...
                    <div
                        ...
                        style={{ scrollbarWidth: "none", msOverflowStyle: "none", background: "var(--color-dark-green)" }}
                    >
                        {recentEvents.length === 0 && (
                            <div className="text-center py-12 font-pixel text-[8px] text-primary/40 text-blink">
                                MONITORING FEED...
                            </div>
                        )}
                        {recentEvents.map((ev, i) => {
                            ...
                            if (ev.type === "strategy_revealed") {
                                return (
                                    <div key={ev.timestamp} className="border-l-2 border-primary pl-3 py-2">
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className="font-pixel text-[7px] text-primary/50">[{time}]</span>
                                            <span className="font-pixel text-[7px] text-primary bg-primary/10 px-1">STRATEGY</span>
                                        </div>
                                        <p className="font-body text-base text-border-green">
                                            <span className="text-primary font-bold">{ev.payload?.agent?.name}</span>
                            ...
                            return (
                                <div key={ev.timestamp} className={`border-l-2 border-primary/20 pl-3 py-1 ${i > 4 ? "opacity-40" : ""}`}>
                                    <span className="font-pixel text-[7px] text-primary/40 block mb-0.5">[{time}] LOG</span>
                                    <p className="font-body text-base text-border-green/70">
                                        {ev.type === "participant_joined" ? (
                                            <><span className="text-primary">{ev.payload?.agent?.name}</span> has entered the arena.</>
                                        ) : (
                    ...
                    {/* Terminal prompt */}
                    <div className="px-4 py-2 border-t border-primary/20" style={{ background: "var(--color-bg-darkest)" }}>
                        <div className="flex items-center gap-2 font-pixel text-[8px] text-primary/50">
